...
def progress(operation, value):
    ...
    if value == 0:
        ...
        status = f"is in progress ({value}%) "
    elif value == 100:
        status = f"has completed ({value}% completed)."
    else:
        print("invalid")
...
def menu(variant=0):
    ...
    if variant == 0 or "":
        ...
        print("""
               ...
               [3] Retrieve confirmed cases, deaths and recoveries for an observation from the database
               [4] Retrieve top 5 countries for confirmed cases from the database from the database 
               [5] Retrieve top 5 countries for deaths for specific observation dates from the database
               """)
        user_response = int(input())
    elif variant == 3:
        print("""
...
def serial_number():
    ...
    """
    print("Please enter the serial number for the record: ")
...
def observation_dates():
    ...
    """
    print("Please enter some observation dates (DD/MM/YYYY,DD/MM/YYYY)")
    text = input()
...
def display_record(record, cols=None):
    ...
    """
    field = []
    if cols is None or cols == field:
        field = record
    else:
        for x in cols:
            field.append(record[x])
    print(field)
